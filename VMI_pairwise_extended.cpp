...
int const Nfeatures = 9;
int const Nclasses = 2;
int const selected_features = 9;

string const file_name = "data/synthdata";

double feat_weights[Nfeatures];
short int training_data[Nsamples][Nfeatures];
...
void feature_selection_discrete_greedy(int n_features) {
	std::fill_n(feat_weights, Nfeatures, 1.0);
	feat_weights[0] = 3.0;
	feat_weights[1] = 2.4;
	feat_weights[2] = 2.2;
	feat_weights[3] = 2.0;
	...
	for (int i = 0; i < Nfeatures; ++i) {
		for (int j = 0; j < Nsamples; ++j) {
			cur_feature[j] = training_data[j][i];
			data_inverse[i][j] = training_data[j][i];
		}
	...
	for (int i = 0; i < Nfeatures; ++i) {
		for (int j = 0; j < Nsamples; ++j) {
			int firstValue = featureDistribution[i].firstStateMap[training_data[j][i]];
		...
		if (initial_step == 0) { //first step: select maximum feature
			...
			for (int i = 0; i < Nfeatures; ++i) {
				if (feature_set.find(i) == feature_set.end()){ //  && selected_feature_set.find(i) != selected_feature_set.end()) {
					...
					double new_weight_sum = last_weight_sum + feat_weights[i];
					for (int j = 0; j < Nsamples; ++j) {
						double tem = 0.0;
						for (int k = 0; k < featureDistribution[i].numSecondStates; ++k) {
							int firstValue = featureDistribution[i].firstStateMap[training_data[j][i]];
							int secondValue = featureDistribution[last_feature].firstStateMap[training_data[j][last_feature]]+k*featureDistribution[last_feature].numFirstStates;
							double conditional_prob = probability_con(distribution, firstValue, secondValue);
							memory_store[i][j][k] = pow(memory_store[i][j][k], last_weight_sum/new_weight_sum); 
							memory_store[i][j][k] *= pow(conditional_prob, feat_weights[i]/new_weight_sum);
...
#	//		cout << max_MI_learned << endl;
		...
		if (initial_step == 0) {
			...
			for (int j = 0; j < Nsamples; ++j) {
				for (int k = 0; k < featureDistribution[selected_feature].numSecondStates; ++k) {
					int firstValue = featureDistribution[selected_feature].firstStateMap[training_data[j][selected_feature]];
		...
		last_weight_sum += feat_weights[selected_feature];
		++initial_step;
